...
The benchmark results demonstrate a clear and consistent performance hierarchy across all experiments: **AVX2 SIMD kernel > scalar x86-64 kernel > C reference kernel**, for every input size and in both Debug and Release configurations. While this ordering is expected in Debug mode, where compiler optimizations are limited, what is more striking is that the **same performance hierarchy persists even in Release mode**, where the C implementation is aggressively optimized by Microsoft's C, C++, and C++/CLI compiler and development tools. Modern compilers in Release mode already perform strength reduction, common-subexpression elimination, loop unrolling, instruction scheduling, and register allocation; yet these optimizations, while significant, still cannot match the performance of hand-written assembly that explicitly uses **256-bit vector registers, packed SIMD instructions, and tailored memory-access patterns**. As a result, the Release-mode C kernel remains fundamentally bottlenecked by scalar computation, because each trajectory is still processed one at a time.

In contrast, the **x86-64 scalar assembly kernel** removes high-level overhead by streamlining loads, stores, and arithmetic operations, achieving roughly **3× speedup** over C for small to medium input sizes. However, its advantage diminishes for very large `n` (e.g., (2^{23})), dropping to approximately **1.9×**, suggesting that the kernel eventually becomes limited by scalar throughput and memory bandwidth. Although the scalar ASM version is clearly more efficient than its C counterpart, it still computes each trajectory sequentially using scalar SSE instructions, meaning it cannot exploit horizontal parallelism inherent in the problem.

The **AVX2 SIMD kernel**, on the other hand, consistently delivers a **4.4×–4.7× speedup** over the C implementation across all input sizes and build modes. These gains arise not only from writing in assembly, but from **algorithmically restructuring the computation to operate on four trajectories at once**, mapping perfectly onto 256-bit YMM registers. This design eliminates three-quarters of the loop-carried overhead, reduces branch pressure, improves instruction-level parallelism, and maximizes the arithmetic density per memory access. As the input size grows, the SIMD kernel’s advantage becomes even more pronounced: compared to the scalar ASM kernel, the AVX2 version is **1.5× faster at smaller sizes** and as much as **2.3× faster at the largest test size**, demonstrating superior scalability with respect to `n`.

Perhaps the most important insight from these results is that **compiler optimization alone is insufficient to achieve true data-parallel speedup** for algorithms like Heun’s method applied to many independent trajectories. Compilers can optimize scalar loops, but they cannot automatically restructure the entire algorithm into an SoA layout, load 4 independent trajectories concurrently, coordinate predictor and corrector stages across packed vectors, or manage register-spilled intermediate values across multiple pipeline stages. These are algorithm-level transformations that require human-guided rethinking of the computation, not just instruction-level tuning. Thus, the AVX2 SIMD kernel’s performance superiority—even against Release-mode C—is not simply due to “faster assembly,” but due to **a fundamentally more parallel algorithmic design that the C compiler is not able to infer on its own**.

In summary, these experiments show that while scalar hand-written assembly yields substantial improvements over C, **true SIMD parallelization is responsible for the majority of the speedup**. By exploiting the independence of trajectories and restructuring the computation to run four trajectories in parallel with AVX2, the SIMD kernel achieves durable, scalable, and architecture-aligned performance gains that remain unattainable for the C implementation, even under the most aggressive compiler optimizations.
